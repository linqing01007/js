...
var calculateMinimumHP = function(dungeon) {
   /*
	假设dp[i][j]表示在[i, j]位置的最低血量，
	1.如果dungeon[i][j] > 0,dp[i][j] = 1,
	2.如果dungeon[i][j] < 0, dp[i][j] = Math.min(dp[i + 1][j], dp[i][j + 1]) - dungeon[i][j]
	所以 dp[i][j] = Math.max(1, Math.min(dungeon[i + 1][j], dungeon[i][j + 1]) - dungeon[i][j]);
	*/
